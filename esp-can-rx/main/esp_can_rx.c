 ...
 *
 * Based on ESP-IDF examples/peripherals/twai/twai_network/twai_listen_only
 * Target: ESP32-S3
 *
 * Runs in normal mode (not listen-only) so the node can ACK frames from
 * esp-can-tx on a two-node bus.
 */
...
/* Message IDs (must match esp-can-tx) */
...
void app_main(void)
{
    printf("===================ESP-CAN-RX (ESP32-S3) Starting...===================\n");
    ...
    /* Normal mode so this node can ACK frames from esp-can-tx */
    ...
    ESP_LOGI(TAG, "TWAI start receiving...");
