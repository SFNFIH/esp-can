...
4. 看到「ESP-CAN 监视器」页面；每个 CAN ID 固定一行，同 ID 新帧只刷新数据
...
- 连接状态：已连接 / 断开重试
- 暂停 / 清空
- 「只看扩展帧」过滤（不匹配行隐藏，仍按 ID 聚合）
- 按 CAN ID 固定行：同 ID 原地更新 DLC / 标志 / 数据 / 序号 / 时间；新 ID 按数值插入；数据变化短暂高亮
- 唯一 ID 软上限 256（超出不再新增行，状态栏提示）
- 表格列：ID、DLC、标志、数据、序号、时间ms
